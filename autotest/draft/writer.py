...
def visit_create(model_name: str, model: Model, func_defs: Dict[str, ast.FunctionDef]):
    ...
    for field in model.fields:
        if field.ty == "text" or field.ty == "varchar":
            ...
            keywords.append(
                ast.keyword(arg=field.name, value=ast.Constant(value="text"))
            )
        else:
            keywords.append(
                ast.keyword(
                    ...
                    value=ast.Call(
                        func=ast.Name(
                            id=f"helper_create_{field.ty.lower()}", ctx=ast.Load()
                        ),
                        args=[],
                        keywords=[],
                    ),
    ...
    body.append(
        ast.Assign(
            targets=[ast.Name(id=model_id, ctx=ast.Store())],
            value=ast.Call(
                func=ast.Attribute(
                    value=ast.Attribute(
    ...
    func_defs[model_name] = ast.FunctionDef(
        name=f"helper_create_{model_id}", args=[], body=body, decorator_list=[]
    )


def topological_sort(
    nodes: Dict[str, object], deps: DefaultDict[str, list[str]], visit, acc
):
    """
...
def make_helper_create(models: Dict[str, Model], deps: DefaultDict[str, List[str]]):
    ...
    root = ast.Module(body=list(func_defs.values()), type_ignores=[])
